...
export default function ProjectCard({ project, onClick, onDelete, isOpen }: ProjectCardProps) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="card-panel p-5 transition-all cursor-pointer group overflow-hidden text-left w-full"
    ...
    >
      <div className="flex items-start justify-between">
        ...
        {/* Right column: open badge + delete */}
        <div className="ml-2 flex flex-col items-end gap-1 flex-shrink-0">
          {isOpen && (
            <span className="px-1.5 py-0.5 bg-spectrum-green/20 text-spectrum-green text-[10px] font-mono font-semibold rounded">
              OPEN
            </span>
          )}
          {onDelete && (
            <button
              ...
              className="p-1 text-ink-muted hover:text-error opacity-0 group-hover:opacity-100 transition-all"
