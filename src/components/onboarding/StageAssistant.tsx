...
export default function StageAssistant({ onComplete }: StageAssistantProps) {
  ...
  const [sessionId, setSessionId] = useState<string>(`mc-${Date.now()}`);
  ...
  // Poll status while GitHub commands are running (runs in Terminal.app, no PTY session to listen to)
  useEffect(() => {
    if (runningCommand !== 'auth-github' && runningCommand !== 'install-github') return;
    ...
    const interval = setInterval(async () => {
      try {
        ...
        const resolved = runningCommand === 'auth-github'
  ...
  const runCommand = (command: string, label: string) => {
    // GitHub commands open in Terminal.app (the in-app terminal doesn't work for gh auth)
    if (label === 'auth-github' || label === 'install-github') {
      runInExternalTerminal(command, label);
      return;
    }
    const newSessionId = `mc-${Date.now()}`;
  ...
  const runInExternalTerminal = async (command: string, label: string) => {
    setRunningCommand(label);
    setCommandResult(null);
    try {
      await window.api.setup.openInTerminal(command);
      // Terminal.app opened — we'll poll for status to detect completion
    } catch (err) {
  ...
  // Ask Assistant mini-chat handler
  const handleChatSend = async () => {
    ...
    try {
      ...
      const prompt = `You are a friendly setup assistant for Mission Control. The user is setting up development tools (Claude Code and GitHub CLI) on their Mac. Help them troubleshoot setup issues. Be concise and practical.\n\nUser: ${userMsg.content}`;
  ...
  const items = [
    {
      ...
      description: 'The AI that powers Mission Control',
    ...
    },
    {
      ...
      description: 'Opens Terminal — follow the prompts to connect',
  ...
  return (
    <div className="max-w-xl w-full">
      ...
      <div className="flex flex-col items-center mb-6">
        ...
        <div className="card-panel p-4 max-w-md text-center">
          <p className="text-sm text-ink leading-relaxed">
            I'm your project assistant — I'll scan your codebase, find issues, and help you ship fixes. Let's get set up.
          </p>
        </div>
      </div>
      ...
      {(runningCommand === 'auth-github' || runningCommand === 'install-github') && !showTerminal && (
        <div className="mt-4 card-panel p-4">
          <div className="flex items-center space-x-3">
            <div className="w-4 h-4 border-2 border-accent border-t-transparent animate-spin flex-shrink-0" />
            <div>
              <p className="text-sm text-ink font-medium">Complete setup in Terminal</p>
              <p className="text-xs text-ink-muted mt-0.5">
                Follow the prompts in the Terminal window that just opened. This screen will update automatically when you're done.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Navigation row: Ask Assistant + Recheck on left, Continue on right */}
      <div className="flex justify-between items-center mt-4">
        <div className="flex items-center space-x-3">
          ...
          >
            <div className="w-5 h-5 rounded-full overflow-hidden border-2 border-accent">
              <img src={mcAvatar} alt="Assistant" className="w-full h-full object-cover scale-[1.3] translate-y-[15%]" />
            </div>
            <span>{showChat ? 'CLOSE CHAT' : 'ASK ASSISTANT'}</span>
          </button>
          <button
      ...
      {/* Ask Assistant mini-chat */}
      {showChat && (
        <div className="mt-4 card-panel overflow-hidden">
          <div className="p-3 border-b border-border flex items-center space-x-2">
            <div className="w-6 h-6 rounded-full overflow-hidden border-2 border-accent">
              <img src={mcAvatar} alt="Assistant" className="w-full h-full object-cover scale-[1.3] translate-y-[15%]" />
            </div>
            <span className="text-sm font-sans font-medium text-ink">Ask Assistant</span>
